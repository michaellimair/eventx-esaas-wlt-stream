...
export default async function handler(
  ...
  const endUrl = `${EVENTX_ESAAS_BASE_URL}/events/${eventId}/wlt-sessions/${webinarResp.data.wltSessionId}/end`;
  const resp = await axios.post<EventXWebinarSessionMeta>(endUrl, {}, {
    headers: {
      cookie: `esaas_access_token=${cookies.esaas_access_token ?? ''}`,
    },
  });
