...
export default async function handler(
  ...
  const pauseUrl = `${EVENTX_ESAAS_BASE_URL}/events/${eventId}/wlt-sessions/${webinarResp.data.wltSessionId}/pause`;
  const resp = await axios.post<EventXWebinarSessionMeta>(pauseUrl, {}, {
    headers: {
      cookie: `esaas_access_token=${cookies.esaas_access_token ?? ''}`,
    },
  });
